# ADR NNNN: Decision Title

<!-- DOC-NAV:START -->
[Home](../../README.md) · [Docs](../README.md) · [Start](../GETTING_STARTED.md) · [Implement](../IMPLEMENTATION_GUIDE.md) · [Architecture](../ARCHITECTURE.md) · [Test](../TEST_STRATEGY.md) · [Interview](../INTERVIEW_PREP.md)
<!-- DOC-NAV:END -->


- Status: Proposed
- Date: YYYY-MM-DD
- Owners: @OWNER

## Context

Describe the engineering problem, constraints, and forces.

## Decision

State the decision clearly.

## Alternatives considered

### Alternative A

Benefits and drawbacks.

### Alternative B

Benefits and drawbacks.

## Consequences

### Positive

- consequence

### Negative

- consequence

## Verification

Explain how the decision will be tested or reviewed.

## Revisit conditions

State what future requirement or evidence would cause this decision to be reconsidered.

---

<!-- DOC-FOOTER:START -->
[Documentation index](../README.md) · [Back to top](#adr-nnnn-decision-title)
<!-- DOC-FOOTER:END -->
