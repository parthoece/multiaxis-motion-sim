# References

<!-- DOC-NAV:START -->
[Home](../README.md) · [Docs](README.md) · [Start](GETTING_STARTED.md) · [Implement](IMPLEMENTATION_GUIDE.md) · [Architecture](ARCHITECTURE.md) · [Test](TEST_STRATEGY.md) · [Interview](INTERVIEW_PREP.md)
<!-- DOC-NAV:END -->


## Industry context

- NIST, *Digital Twins for Advanced Manufacturing*:  
  https://www.nist.gov/programs-projects/digital-twins-advanced-manufacturing
- Siemens Digital Industries Software, virtual commissioning case studies and machine-development resources:  
  https://resources.sw.siemens.com/en-US/case-study-wipro-pari/  
  https://resources.sw.siemens.com/en-US/case-study-idc/
- Industrial Technology Research Institute, Taiwan industrial digital-transformation and interoperability resources:  
  https://www.itri.org.tw/
- Taiwan Ministry of Economic Affairs, industrial and smart-manufacturing programs:  
  https://www.moea.gov.tw/

## Runtime and framework

- Microsoft, .NET support policy:  
  https://dotnet.microsoft.com/platform/support/policy
- Microsoft, `global.json` SDK selection:  
  https://learn.microsoft.com/dotnet/core/tools/global-json
- Microsoft.Data.Sqlite package:  
  https://www.nuget.org/packages/Microsoft.Data.Sqlite/

## LinuxCNC

- LinuxCNC INI configuration:  
  https://linuxcnc.org/docs/html/config/ini-config.html
- LinuxCNC homing configuration:  
  https://linuxcnc.org/docs/stable/html/config/ini-homing.html
- LinuxCNC G38 probing:  
  https://linuxcnc.org/docs/stable/html/gcode/g-code.html
- LinuxCNC motion component:  
  https://linuxcnc.org/docs/html/man/man9/motion.9.html
- LinuxCNC kinematics concepts:  
  https://linuxcnc.org/docs/stable/html/motion/5-axis-kinematics.html

## Open-source maintenance

- GitHub Community Standards:  
  https://docs.github.com/communities/setting-up-your-project-for-healthy-contributions
- GitHub Actions for .NET:  
  https://docs.github.com/actions/tutorials/build-and-test-code/net
- REUSE Specification:  
  https://reuse.software/spec/
- OpenSSF Best Practices:  
  https://www.bestpractices.dev/

---

<!-- DOC-FOOTER:START -->
[Documentation index](README.md) · [Back to top](#references)
<!-- DOC-FOOTER:END -->
